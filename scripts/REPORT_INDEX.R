...
write.table(df.panels[,c("tissue","modality","n")],quote=F,row.names=F,col.names=F,sep=' | ',file=fout,append=T)
# ----
...
genes.nfo = read.table("data/genes.nfo",as.is=T)
df.genes$n.assoc = genes.nfo[ match( df.genes$gene , genes.nfo[,1] ) , 2 ]
genes.nfo = read.table("data/genes.models.nfo",as.is=T)
