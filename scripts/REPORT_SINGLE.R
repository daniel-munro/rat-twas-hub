...
df.panels <- data.frame("tissue" = tbl.panels$TISSUE,
...
df.cur.models <- data.frame("tissue" = tbl.panels$TISSUE,
                            "modality" = tbl.panels$MODALITY,
                            "num.hits" = rep(0, N.models),
                            "pct.hits" = rep(0, N.models),
...
if (file.info(paste(tbl.traits$OUTPUT[i], ".post.report", sep = ''))$size != 0) {
    cur.clumps <- read.table(paste(tbl.traits$OUTPUT[i], ".post.report", sep = ''), as.is = TRUE, head = TRUE)
    ...
    for (ii in 1:nrow(cur.clumps)) {
        cur.genes.tbl <- read.table(paste(cur.clumps.files[ii], ".genes", sep = ''), head = TRUE, as.is = TRUE)
    ...
    write.table(format(df.cur.models, digits = 2), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = ' | ', file = fout, append = TRUE)
    cat('{: #panels}\n\n', file = fout, append = TRUE)
}
