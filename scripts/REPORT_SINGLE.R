...
tbl.traits = read.table("data/traits.par",as.is=T,head=T,sep='\t',quote="")
...
df.panels = data.frame("tissue" = tbl.panels$TISSUE, "modality" = tbl.panels$MODALITY, "num.hits" = rep(0,N.models), "num.traits" = rep(0,N.models), row.names=tbl.panels$ID)

cat("reading data/all.models.par\n")
...
traits.nfo = read.table("data/traits.par.nfo",as.is=T,head=T)
...
cur = cur[!is.na(cur$TWAS.P),] # Added -DM
n = nrow(cur)
...
df.cur.models = data.frame("tissue"=tbl.panels$TISSUE , "modality"=tbl.panels$MODALITY, "num.hits" = rep(0,N.models), "pct.hits" = rep(0,N.models), "avg.chisq" = rep(NA,N.models), row.names=tbl.panels$ID)
for ( m in 1:N.models ) {
...
for ( ii in 1:nrow(cur.clumps) ) {
	...
	cur.genes.tbl$MODALITY = tbl.panels$MODALITY[m]
...
write.table(format(cur.genes.tbl[,c("NUM","TISSUE","MODALITY","GENE","HSQ","EQTL.R2","MODEL","NWGT","MODELCV.R2","MODELCV.PV","EQTL.GWAS.Z","TWAS.Z","TWAS.P","TOP.SNP.COR","COLOC.PP3","COLOC.PP4","JOINT")],digits=2),quote=F,row.names=F,col.names=F,sep=' | ',file=fout.clump,append=T)
	cat( '{: #models}\n\n' , file=fout.clump,append=T)
}
