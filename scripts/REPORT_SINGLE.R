...
cat("reading genes.par\n")
gene_names <- read.table("genes.par", as.is = TRUE, head = TRUE, sep = '\t')
...
df.panels = data.frame("tissue" = tbl.panels$TISSUE, "rnapheno" = tbl.panels$RNAPHENO, "num.hits" = rep(0,N.models), "num.traits" = rep(0,N.models), row.names=tbl.panels$ID)
...
tbl.models.pos$PANEL = paste( tbl.panels$TISSUE , " | " , tbl.panels$RNAPHENO , sep='' )[ m ] 
...
df.cur.models = data.frame("tissue"=tbl.panels$TISSUE , "rnapheno"=tbl.panels$RNAPHENO, "num.hits" = rep(0,N.models), "pct.hits" = rep(0,N.models), "avg.chisq" = rep(NA,N.models), row.names=tbl.panels$ID)
for ( m in 1:N.models ) {
...
for ( ii in 1:nrow(cur.clumps) ) {
	...
	cur.genes.tbl$GENE = paste( "*[" , gene_names[cur.genes.tbl$ID] , "]({{ site.baseurl }}genes/" , cur.genes.tbl$ID , ")*" , sep='' )
	...
	cur.genes.tbl$RNAPHENO = tbl.panels$RNAPHENO[m]
	cur.genes = sort(unique(cur.genes.tbl$ID[ cur.genes.tbl$JOINT ]))
	cur.genes = paste( '*' , paste( "[" , gene_names[cur.genes] , "]({{ site.baseurl }}genes/" , cur.genes , ")" , sep='' , collapse=' ') , '*' , sep='' )
	...
	cat( "| # | Tissue | RNA pheno | Gene | h2 | eQTL R2 | model | # weights | model R2 | model R2 P | eQTL GWAS Z | TWAS Z | TWAS P | Top SNP corr | PP3 | PP4 | joint |","| --- |",sep='\n',file=fout.clump,append=T)
...
write.table(format(cur.genes.tbl[,c("NUM","TISSUE","RNAPHENO","GENE","HSQ","EQTL.R2","MODEL","NWGT","MODELCV.R2","MODELCV.PV","EQTL.GWAS.Z","TWAS.Z","TWAS.P","TOP.SNP.COR","COLOC.PP3","COLOC.PP4","JOINT")],digits=2),quote=F,row.names=F,col.names=F,sep=' | ',file=fout.clump,append=T)
	cat( '{: #models}\n\n' , file=fout.clump,append=T)
}
...
cat( "| tissue | RNA pheno | # hits | % hits/tests | avg chisq |","| --- |",sep='\n',file=fout,append=T)
